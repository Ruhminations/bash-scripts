...
echo "$2 /bedrock_bucket fuse.s3fs use_path_request_style,passwd_file=/root/.s3fs-password,url=https://$3.compat.objectstorage.$4.oraclecloud.com,endpoint=$4 kernel_cache,multipart_size=128,parallel_count=50,multireq_max=100,max_background=1000,_netdev" | sudo tee -a /etc/fstab
